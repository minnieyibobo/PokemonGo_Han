...
def check_ifActive(player_name):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    c.execute("SELECT * FROM active_players WHERE player_name='"+player_name+SINGLE_COMMA_STRING)
    result = len(c.fetchall())>0
    conn.close()
    return result
...
def get_activeUsers():
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    c.execute("SELECT * FROM active_players")
    result = c.fetchall()
    return result

def active_user(player_name):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    if(check_ifActive(player_name) == False):
        c.execute("INSERT INTO active_players VALUES ('"+player_name+"','" +str(0)+ SINGLE_COMMA_STRING +")")
    conn.commit()
    conn.close()

def deactive_user(player_name):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    if(check_ifActive(player_name) == True):
        c.execute("DELETE FROM active_players WHERE player_name=" + "'" +player_name+"'")
    conn.commit()
    conn.close()

# use pokemon to fight
def usingPokemon(player_name, pokemon_name):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    if(check_ifActive(player_name) == True):
        c.execute("UPDATE active_players SET usingPokemon=" + SINGLE_COMMA_STRING +pokemon_name +SINGLE_COMMA_STRING+" WHERE player_name=" + SINGLE_COMMA_STRING +player_name+ SINGLE_COMMA_STRING)
    conn.commit()
    conn.close()

def check_fightingPokemon(player_name):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    c.execute("SELECT usingPokemon FROM active_players WHERE player_name='"+player_name+SINGLE_COMMA_STRING)
    result = c.fetchall()[0]
    conn.close()
    return result
...
def get_player_pokemons(player_userName):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    c.execute("SELECT pokedex_ID FROM player_pokemon WHERE player_name='" + str(player_userName) + "'")
    result=c.fetchall()
    conn.commit()
    conn.close()
    return result
