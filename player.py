...
class Player(object):
    ...
    def enter_battle_module(self):
        all_users = database.get_activeUsers()
        print (all_users)

        if(len(all_users) == 0 or len(all_users) == 1):
            print ("No one is playing, get some friends yoyo!!")
            return

        # randomly pick a user to fight
        print ("I am randomly matching you a player")
        fight_with_player = random.choice(all_users)
        while (fight_with_player[0] == self.username):
            fight_with_player = random.choice(all_users)
        fight_with_player = fight_with_player[0]
        print ("You will fight with " + fight_with_player)

        # here are all your pokemons, pick one to fight
        # TODO: the two might fight different people ... fix it later.
        index=0
        print ("Tell me which pokemon you want to use, index please!")
        for pokemon in self.pokemons_in_hand:
            print (str(index) +": " + pokemon.name)
            index +=1
        selectedPokemon = self.pokemons_in_hand[int(input())]
        database.usingPokemon(self.username, selectedPokemon.name)
        print ("You are fighting " + fight_with_player+" using pokemon " + selectedPokemon.name)

        # check if my partner is ready
        # TODO: this piece of code should not be here...but I am too lazy to organize it...
        against_pokemon = database.check_fightingPokemon(fight_with_player)
        while(len(against_pokemon) == 0):
            time.sleep(5)
        print ("You enemy is using " + against_pokemon[0])
        print ("both of you are ready, you take an action, 0:fast move; 1: special move; 2: end fight!")
        enemyPokemon = Pokemon(against_pokemon)
        move = int(input())
        while(move<2):
            if move == 0:
                selectedPokemon.attack(selectedPokemon.fast_move, enemyPokemon)
            if move == 1:
                selectedPokemon.attack(selectedPokemon.special_move, enemyPokemon)
            if(selectedPokemon.cp <= 0 or enemyPokemon.cp <= 0):
                print ("either one of you are dead, dude stop fighting!")
                move = 2
